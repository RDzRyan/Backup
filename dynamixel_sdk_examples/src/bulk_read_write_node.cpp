...
// Default setting
#define DXL1_ID 22                  // DXL1 ID
#define DXL2_ID 52                  // DXL2 ID
...
int main(int argc, char **argv)
{
  ...
  uint8_t addr_goal_item[2];
  uint8_t len_goal_item[2];
  uint8_t dmx_id[18] = {11, 12, 13, 21, 22, 23, 31, 32, 33, 41, 42, 43, 51, 52, 53, 61, 62, 63};
  ...
  if (!portHandler->setBaudRate(BAUDRATE))
  {
    ...
    return -1;
		...
		uint32_t position = 500; // Convert int32 -> uint32
    ...
    for (int i = 0; i <= 18; i++)
    {
      dxl_addparam_result = groupBulkWrite.addParam(dmx_id[i], addr_goal_item[0], len_goal_item[0], param_goal_position);
			ROS_INFO("Step %d", i);
    }
    dxl_comm_result = groupBulkWrite.txPacket();
		if (dxl_comm_result == COMM_SUCCESS)
  	{
    	ROS_INFO("Success Write");
  	}
  	else
  	{
  	  ROS_INFO("Failed to set position! Result: %d", dxl_comm_result);
