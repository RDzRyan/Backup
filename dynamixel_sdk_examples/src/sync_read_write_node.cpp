 ...
 * $ rostopic pub -1 /sync_set_position dynamixel_sdk_examples/SyncSetPosition "{id1: 11, id2: 12, position1: 500, position2: 500}"
 * $ rostopic pub -1 /sync_set_position dynamixel_sdk_examples/SyncSetPosition "{id1: 11, id2: 31, id3: 51, position1: 500, position2: 500, position3: 500}"
...
void addWrite (uint32_t posisi, uint8_t id)
{
  int dxl_addparam_result = false;
  uint8_t param_goal_position[4];
  param_goal_position[0] = DXL_LOBYTE(DXL_LOWORD(posisi));
  param_goal_position[1] = DXL_HIBYTE(DXL_LOWORD(posisi));
  param_goal_position[2] = DXL_LOBYTE(DXL_HIWORD(posisi));
  param_goal_position[3] = DXL_HIBYTE(DXL_HIWORD(posisi));
...
void syncSetPositionCallback(const dynamixel_sdk_examples::SyncSetPosition::ConstPtr & msg)
{
  ...
  addWrite((unsigned int)msg->position1,(uint8_t)msg->id1);
  addWrite((unsigned int)msg->position2,(uint8_t)msg->id2);
...
{
	uint8_t dxl_id[18] = {11,12,13,21,22,23,31,32,33,41,42,43,51,52,53,61,62,63};
	uint32_t step[10][18] = {
  ...
  groupSyncWrite.clearParam();
	...
	while(1){
		for (int i = 4; i<10; i++){
			...
			dxl_comm_result = groupSyncWrite.txPacket();
  		if (dxl_comm_result == COMM_SUCCESS) {
    		ROS_INFO("setPosition Success %d", i);
  		} 
			else {
    		ROS_ERROR("Failed to set position! Result: %d", dxl_comm_result);
  		}
